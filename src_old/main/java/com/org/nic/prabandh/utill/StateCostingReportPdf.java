...
public class StateCostingReportPdf{
	...
	public ResponseEntity<?> downloadCostingReportPdf(String planYear, Map<Integer, Map<Integer, Map<Integer, Map<Integer, Map<Integer, List<ProposedCosting>>>>>> costingReportMap, String regionName) throws IOException {
		...
		doc.add(CommonMethod.createHeadingParaGraph(regionName, 20f, 0f, 35, new DeviceRgb(165, 42, 42)));
		doc.add(CommonMethod.createHeadingParaGraph(planYear, 20f, 0f, 40, new DeviceRgb(165, 42, 42)));
		doc.add(CommonMethod.createHeadingParaGraph("(Prepared by - " + regionName + ")", 40f, 0f, 15, new DeviceRgb(12, 49, 99)));
	...
	private Table getCostingReportStateWise(Document doc, Map<Integer, Map<Integer, Map<Integer, Map<Integer, Map<Integer, List<ProposedCosting>>>>>> groupedByFiveAttributes, Integer schemeKey,
		...
		if (schemeKey != 555555) {
			...
			CommonMethod.createDataCelltableHeaderWithBgBlue(table, "Proposed Physical Quantity", 1, 1, 10f, TextAlignment.CENTER);
			CommonMethod.createDataCelltableHeaderWithBgBlue(table, "Proposed Unit Cost", 1, 1, 10f, TextAlignment.CENTER);
			CommonMethod.createDataCelltableHeaderWithBgBlue(table, "Proposed Financial Amount (In Lakhs)", 1, 1, 10f, TextAlignment.CENTER);
			CommonMethod.createDataCelltableHeaderWithBgBlue(table, "Coordinator Remarks", 1, 1, 10f, TextAlignment.CENTER);
		}
						...
						for (Map.Entry<Integer, List<ProposedCosting>> finalEntry : deepestValue.entrySet()) {
							...
							for (ProposedCosting listObj : finalValueList) {
								...
								if (listObj.getSchemeId() == 555555) {
									...
									CommonMethod.createDataCellTotalWithBorderRight(table, df.format(listObj.getProposedPhysicalQuantity()), 1, 1,9);
