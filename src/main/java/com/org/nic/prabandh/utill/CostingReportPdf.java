...
public class CostingReportPdf {
	...
	public byte[] addFooterAndPageNumbers(byte[] pdf, String stateName, String planYear) throws Exception {
		...
		for (int i = 1; i <= numberOfPages; i++) {
			...
			if (i > 1) {
				new Canvas(pdfCanvas, pdfDoc, pageSize).setFont(PdfFontFactory.createFont(StandardFonts.HELVETICA_BOLD)).setFontSize(10).setFontColor(new DeviceRgb(165, 42, 42))
						.showTextAligned("F. Y. - " + planYear, 37, 820, TextAlignment.LEFT, VerticalAlignment.MIDDLE, 0)
						.showTextAligned("Budget Demand  - " + stateName, 433, 820, TextAlignment.LEFT, VerticalAlignment.MIDDLE, 0);
			}
	...
	public ResponseEntity<?> downloadCostingReportPdf(String planYear, Map<Integer, Map<Integer, Map<Integer, Map<Integer, Map<Integer, List<ProposedCosting>>>>>> costingReportMap, String stateName,
			Map<Integer, Double[]> recurNonrecurFinAmtTot, Optional<MastStatesTentative> stateTentive) throws IOException {
		...
		doc.add(CommonMethod.createHeadingParaGraph("Costing Sheet", 80f, 0f, 25, new DeviceRgb(165, 42, 42)));
		doc.add(CommonMethod.createHeadingParaGraph("(Samagra Shiksha)", 0f, 0f, 30, new DeviceRgb(165, 42, 42)));
		doc.add(CommonMethod.createHeadingParaGraph("of", 0f, 0f, 20, new DeviceRgb(165, 42, 42)));
		doc.add(CommonMethod.createHeadingParaGraph(stateName, 0f, 0f, 25, new DeviceRgb(165, 42, 42)));
		doc.add(CommonMethod.createHeadingParaGraph(planYear, 0f, 0f, 30, new DeviceRgb(165, 42, 42)));

		doc.add(CommonMethod.createHeadingParaGraph("(Prepared by - " + stateName + ")", 0f, 0f, 15, new DeviceRgb(165, 42, 42)));
		// first page paragraph---End------------

		if (costingReportMap.size() > 1) {
			// first page Summary at a Glance table----Start-----------
			doc.add(CommonMethod.createHeadingParaGraph("Summary at a Glance", 20f, 0f, 15, new DeviceRgb(0, 0, 0)));
			Table summaryGlance = getSummaryGlance(doc, costingReportMap, recurNonrecurFinAmtTot, stateTentive);
			doc.add(summaryGlance);
			// first page Summary at a Glance table----end-----------
		...
		try {
			bytes = addFooterAndPageNumbers(bytes, stateName, planYear);
		} catch (Exception e) {
			e.printStackTrace();
		}
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Disposition", "inline; filename=Costing Sheet for " + stateName + ".pdf");
		return ResponseEntity.ok().headers(headers).contentType(MediaType.APPLICATION_PDF).body(bytes);
	}

	private Table getSummaryGlance(Document doc, Map<Integer, Map<Integer, Map<Integer, Map<Integer, Map<Integer, List<ProposedCosting>>>>>> groupedByFiveAttributes,
			Map<Integer, Double[]> recurNonrecurFinAmtTot, Optional<MastStatesTentative> stateTentive) throws IOException {
		float[] columnWidths = { 2f, 1f, 1f, 1f };
		Table table = new Table(UnitValue.createPercentArray(columnWidths));
		table.setWidth(UnitValue.createPercentValue(100));

		CommonMethod.createDataCellBoldWithBackGroundColor(table, "Particulars", 1, 2, 12f, TextAlignment.LEFT);
		CommonMethod.createDataCellBoldWithBackGroundColor(table, "Amount (In Lakhs)", 3, 1, 12f, TextAlignment.CENTER);
		CommonMethod.createDataCellBoldWithBackGroundColor(table, "Recurring", 1, 1, 12f, TextAlignment.CENTER);
		CommonMethod.createDataCellBoldWithBackGroundColor(table, "Non-Recurring", 1, 1, 12f, TextAlignment.CENTER);
		CommonMethod.createDataCellBoldWithBackGroundColor(table, "Total", 1, 1, 12f, TextAlignment.CENTER);
		...
		if (recurNonrecurFinAmtTot != null && recurNonrecurFinAmtTot.size() > 0) {
			Double[] recurNonrecurFinAmt = recurNonrecurFinAmtTot.get(1);
			if (recurNonrecurFinAmtTot.get(1) != null && recurNonrecurFinAmt.length > 0) {
				elementaryRec = recurNonrecurFinAmtTot.get(1)[0];
				elementaryNonRec = recurNonrecurFinAmtTot.get(1)[1];
				elementaryRecNonRecTot = elementaryRec + elementaryNonRec;
			}
			if (recurNonrecurFinAmtTot.get(2) != null && recurNonrecurFinAmt.length > 0) {
				secondaryRec = recurNonrecurFinAmtTot.get(2)[0];
				secondaryNonRec = recurNonrecurFinAmtTot.get(2)[1];
				secondaryRecNonRecTot = secondaryRec + secondaryNonRec;
			}
			if (recurNonrecurFinAmtTot.get(3) != null && recurNonrecurFinAmt.length > 0) {
				teacherRec = recurNonrecurFinAmtTot.get(3)[0];
				teacherNonRec = recurNonrecurFinAmtTot.get(3)[1];
				teacherRecNonRecTot = teacherRec + teacherNonRec;
			}
		}

		CommonMethod.createDataCellBoldLeft(table, "Elementary Education", 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(elementaryRec), 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(elementaryNonRec), 1, 1, 10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(elementaryRecNonRecTot), 1, 1, 11);

		CommonMethod.createDataCellBoldLeft(table, "Secondary Education", 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(secondaryRec), 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(secondaryNonRec), 1, 1, 10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(secondaryRecNonRecTot), 1, 1, 11);

		CommonMethod.createDataCellBoldLeft(table, "Teacher Education", 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(teacherRec), 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(teacherNonRec), 1, 1, 10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(teacherRecNonRecTot), 1, 1, 11);
		...
		CommonMethod.createDataCellBoldLeft(table, "Grant Total", 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(grantTotalRecur), 1, 1, 10);
		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(grantTotalNonrecur), 1, 1, 10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(grantTotaltotalRecurNonrecur), 1, 1, 11);

		Double parcentState = stateTentive.get().getCenterSharePercent();
		Double reParcentState = (100 - stateTentive.get().getCenterSharePercent());

		CommonMethod.createDataCellBoldLeft(table, "Central Share", 3, 1, 10);
//		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(grantTotalRecur*(parcentState/100)), 1, 1,10);
//		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(grantTotalNonrecur*(parcentState/100)), 1, 1,10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(grantTotaltotalRecurNonrecur * (parcentState / 100)), 1, 1, 10);

		CommonMethod.createDataCellBoldLeft(table, "State Share", 3, 1, 10);
//		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(grantTotalRecur*(reParcentState/100)), 1, 1,10);
//		CommonMethod.createDataCellCategoryWithBorderRight(table, df.format(grantTotalNonrecur*(reParcentState/100)), 1, 1,10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(grantTotaltotalRecurNonrecur * (reParcentState / 100)), 1, 1, 10);

		CommonMethod.createDataCellBoldLeft(table, "Tentative Proposed Release", 1, 3, 10);
		CommonMethod.createDataCellBoldLeft(table, "State Share", 2, 1, 10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(stateTentive.get().getTentativeStateShare()), 1, 1, 11);
		CommonMethod.createDataCellBoldLeft(table, "Central Share", 2, 1, 10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(stateTentive.get().getTentativeCentralShare()), 1, 1, 11);
		CommonMethod.createDataCellBoldLeft(table, "Total Estimates", 2, 1, 10);
		CommonMethod.createDataCellTotalWithBorderRight(table, df.format(stateTentive.get().getTentativeTotalEstimates()), 1, 1, 11);
						...
						for (Map.Entry<Integer, List<ProposedCosting>> finalEntry : deepestValue.entrySet()) {
							...
							for (ProposedCosting listObj : finalValueList) {
								...
								if (listObj.getSchemeId() == 555555) {
									...
									CommonMethod.createDataCellBoldRight(table, "Total of " + listObj.getSchemeName(), 4, 1);
									isTotal = true;
								} else if (listObj.getSubComponentId() == 777777) {
									CommonMethod.createDataCellBoldRight(table, "Total of " + listObj.getMajorComponentName(), 3, 1);
									isTotal = true;
								} else if (listObj.getActivityMasterId() == 888888) {
									CommonMethod.createDataCellBoldRight(table, "Total of " + listObj.getSubComponentName(), 2, 1);
									isTotal = true;
								} else if (listObj.getActivityMasterDetailsId() == 999999) {
									CommonMethod.createDataCellBoldRight(table, "Sub Total", 1, 1);
									isTotal = true;
								} else {
									subActivitySrNo++;
								...
								if (isTotal) {
									if (listObj.getSchemeId() != 555555) {
										CommonMethod.createDataCellBoldRight(table, listObj.getPhysicalQuantity() == null ? "" : listObj.getPhysicalQuantity() + "", 1, 1);
										CommonMethod.createDataCellBoldRight(table, "", 1, 1);
										CommonMethod.createDataCellBoldRight(table, listObj.getFinancialAmount() == null ? "" : df.format(listObj.getFinancialAmount()) + "", 1, 1);
									}
								} else {
